<script lang="ts">
	import { getResizedImage } from '$lib';
	...
	const resizedImage = getResizedImage(posterUrl, 400);
</script>
...
<div class={['relative aspect-2/3 w-80 overflow-hidden rounded-3xl bg-neutral-900 shadow-xl', className]}>
	<img
		...
		alt={title}
		draggable="false"
		class={['absolute inset-0 h-full w-full object-cover transition-opacity duration-700', imageLoaded ? 'opacity-100' : 'opacity-0']}
		onload={() => (imageLoaded = true)}
	/>

	<div
		class={[
			'absolute inset-0 h-full w-full bg-cover bg-center transition-opacity duration-300',
			!imageLoaded ? 'opacity-100' : 'opacity-0',
		]}
		style="background-image: url({resizedImage}); filter: blur(20px); transform: scale(1.1);"
	></div>
